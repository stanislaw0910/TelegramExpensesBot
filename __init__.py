...
URL = 'https://f068-188-243-246-246.ngrok.io'
requests.get('https://api.telegram.org/bot' + TG_API_TOKEN + '/setWebhook?url=' + URL)  # register URL for Webhook using
...
# noinspection SpellCheckingInspection
@bot.message_handler(commands=['DefinedMonthExpenseByCategory', 'definedmonthexpensebycategory', 'DMEC', 'dmec'])
def defined_month_expense(message):
    """
    ...
    if message.from_user.id == OWNER_ID:
        msg=bot.reply_to(message, 'Enter year and month in format YYYY.MM: ')
        bot.register_next_step_handler(msg, defined_month_expense_input)
    else:
        bot.send_message(message.chat.id, "Access denied!!!\nPlease ensure you have right to use this bot!")


def defined_month_expense_input(message):
    """
